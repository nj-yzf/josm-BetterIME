...
**1. 自动切换至中文输入法：**
- 编辑 `name`、`name:zh`、`name:zh-Hans`、`name:zh-Hant`、`alt_name`、`operator` 标签时（可在首选项中自定义）
- 打开 F3「搜索预设组合」对话框时

**2. 默认英文，可手动切换：**
- 其他文本输入框（如 Ctrl+F 查找、其他标签编辑等）

**3. 禁用输入法（快捷键正常工作）：**
- 地图视图、工具栏等非文本组件（Shift/Ctrl+Space 也无法切换至中文）

### 首选项设置

通过 **编辑 → 首选项 (F12)** 打开 BetterIME 设置页，可配置：

- 启用/禁用自动 IME 切换（总开关）
- 启用/禁用 F3 搜索预设对话框中的中文 IME
- 启用/禁用基于标签键的中文 IME 检测
- 自定义触发中文 IME 的标签键列表（添加、删除、重置）

所有设置即时生效，无需重启 JOSM。也可在高级首选项中直接编辑 `betterime.*` 键值。
